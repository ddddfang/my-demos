...
int main(int argc, char *argv[]) {
    ...
    data.playbin = gst_element_factory_make("playbin", "myplaybin");
    if (!data.playbin) {
        g_printerr("Not all element could be created.\n");
        return -1;
    }
    g_object_set(data.playbin, "uri", "file:///home/fang/testvideo/Titanic.ts", NULL);
    ...
    do {
        ...
        } else {    //got not message, means time out. (timeout every 100 ms)
            if (data.playing) {
                gint64 current = -1;
