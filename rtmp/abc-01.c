...
int publish_using_packet(){
    ...
    rtmp->Link.timeout = 5;
    //live关键字来自 /usr/local/nginx/conf/nginx.conf 中对rtmp的配置
    //点播使用 rtmp://127.0.0.1:1935/vod/xxx.flv, 拉取直播流应该就是 rtmp://127.0.0.1:1935/live
    if(!RTMP_SetupURL(rtmp, "rtmp://127.0.0.1:1935/live")) //本次推流 目标服务器 url
    {
    ...
    if (!RTMP_Connect(rtmp, NULL)) {    //连接目标 url
    ...
    RTMPPacket_Alloc(packet, 1024*64);  //64 KBytes
    ...
    while(1)
    {
        ...
        if(!ReadU8(&type, fp))          //type
            break;
        if(!ReadU24(&datalength, fp))   //datasize
            break;
        if(!ReadTime(&timestamp, fp))   //timestamp
            break;
        if(!ReadU24(&streamid, fp))     //stream id
            break;

        if (type != 0x08 && type != 0x09){
            //jump over non_audio and non_video frame，
            //jump over next previousTagSizen at the same time
            fseek(fp, datalength + 4, SEEK_CUR);    //跳转到下一个tag起始处
            continue;
        }
        ...
        if (!RTMP_SendPacket(rtmp, packet, 0)) {    //这是将上面拿到的 flv 信息重新打包发送啊
        ...
        if(!PeekU8(&type, fp))  //看一下下一个tag的类型
            break;
        if(type == 0x09) {                      //下一个tag是video的话
            if(fseek(fp, 11, SEEK_CUR) != 0)    //跳过tag header
                break;
            if(!PeekU8(&type, fp))              //tag data 的第一个字节是 video 参数
                break;
            if(type == 0x17)                    //0x17 代表是关键帧,h264编码
                bNextIsKey = 1;
            else
                bNextIsKey = 0;

            fseek(fp, -11, SEEK_CUR);           //seek回去
        }
    }
